...
public class ExcelReporter {
    ...
    public static ByteArrayInputStream getUserExcelReport(List<User> users) throws IOException {
        ...
        Workbook workbook = new XSSFWorkbook(); // excel dosyası oluşturma

        ByteArrayOutputStream out = new ByteArrayOutputStream();

        Sheet sheet = workbook.createSheet(SHEET_USER); // Sheet objesinin ismi

        Row headerRow = sheet.createRow(0); // header row --> excel içindeki verilerin başlıkları

        // header row oluşturma
        for (int i = 0; i < USER_HEADERS.length; i++) {

            Cell cell = headerRow.createCell(i); // header için cell oluşturma

            cell.setCellValue(USER_HEADERS[i]);// her header cell için değer girme
        ...
        for (User user : users) {
            ...
            for (Role role : user.getRoles()) {
                ...
                joiner.add(role.getRoleType().getName());

            }

            row.createCell(7).setCellValue(joiner.toString());
    ...
    public static ByteArrayInputStream getReservationExcelReport(List<Reservation> reservations) throws IOException{
        ...
        for (Reservation reservation : reservations) {
            ...
            row.createCell(9).setCellValue(reservation.getStatus().toString());
